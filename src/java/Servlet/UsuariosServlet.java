...
public class UsuariosServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("Guardar".equals(Accion))
        {
            ...
            {
                modeUsuario.setId(-1);
                modeUsuario.setCodigo(Codigo);
                modeUsuario.setNombre(Nombre);
                modeUsuario.setUsuario(Clave);
                modeUsuario.setPassword(Password);
                if (controladorUsuarios.Insert(modeUsuario))
                {
                    respuesta = "true";
            ...
            {
                modeUsuario.setId(Integer.valueOf(Id));
                modeUsuario.setCodigo(Codigo);
                modeUsuario.setNombre(Nombre);
                modeUsuario.setUsuario(Clave);
                modeUsuario.setPassword(Password);
                if (controladorUsuarios.Update(modeUsuario))
                {
                    respuesta = "true";
                    response.setCharacterEncoding("UTF-8");
                    response.setContentType("text/plain");
        ...
        if ("Eliminar".equals(Accion))
        {
            ...
            modeUsuario.setUsuario(Clave);
            modeUsuario.setPassword(Password);
            if (controladorUsuarios.Delete(modeUsuario))
            {
                respuesta = "true";
                response.setCharacterEncoding("UTF-8");
                response.setContentType("text/plain");
        ...
        if ("Buscar".equals(Accion))
        {
            ModeloUsuario modeUsuario = new ModeloUsuario();
            ControladorUsuarios controladorUsuarios = new ControladorUsuarios();
            modeUsuario = controladorUsuarios.Select(CodigoBuscar);
...
//            requestDispatcher.forward(request, response);
        ...
        if ("Ingresar".equals(Accion))
        {
            ModeloUsuario modeUsuario = new ModeloUsuario();
            ControladorUsuarios controladorUsuarios = new ControladorUsuarios();
            modeUsuario = controladorUsuarios.Select(Clave, Password);
            //request.setAttribute("modeloUsuario", modeUsuario);
